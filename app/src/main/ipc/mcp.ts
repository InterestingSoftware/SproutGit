...
export function registerMcpHandlers(getWindow: () => BrowserWindow | null, configDb: ConfigDb): void {
  ...
  handle(IPC.MCP_ENSURE_STARTED, async (_e, workspacePath: string) => {
    if (readEnabled(workspacePath)) {
      await startMcpServer(paramsFor(workspacePath), getWindow, configDb);
    }
  ...
  handle(IPC.MCP_SET_ENABLED, async (_e, args: { workspacePath: string; enabled: boolean }) => {
    ...
    if (args.enabled) {
      await startMcpServer(paramsFor(args.workspacePath), getWindow, configDb);
    } else {
