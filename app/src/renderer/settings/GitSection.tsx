...
function commandToken(value: string): string {
  ...
  if (!trimmed) return "";
  const first = tokenizeCommand(trimmed)[0] ?? "";
...
function matchesEditor(editor: EditorInfo, configured: string): boolean {
  const first = tokenizeCommand(configured.trim())[0] ?? "";
  const base = first.split(/[\\/]/).pop() ?? first;
  return base === editor.command || base.startsWith(editor.command);
}
...
function buildMergeToolCommand(tool: GitToolInfo): string | null {
  ...
  if (tool.id === "opendiff")
    return 'opendiff "$LOCAL" "$REMOTE" -merge "$MERGED"';
  return null;
}
...
export function GitSection({ onToast, githubAuth }: Props) {
  ...
  const [githubEmailSuggestions, setGithubEmailSuggestions] = useState<
    GitHubEmailSuggestion[]
  >([]);
    ...
    void Promise.all([
      ...
      .then(
        ([
          ...
          if (
            configEditor &&
            !detectedEditors.some(
              (e: EditorInfo) => e.installed && matchesEditor(e, configEditor),
            )
          ) {
  ...
  useEffect(() => {
    if (!githubAuth?.authenticated) {
      setGithubEmailSuggestions([]);
      return;
    }
    setGithubEmailsLoading(true);
    void api
      .githubListEmails()
      .then((emails: GitHubEmailSuggestion[]) =>
        setGithubEmailSuggestions(emails),
      )
  ...
  const installedDiffTools = gitTools.filter(
    (t) => t.installed && t.supportsDiff,
  );
  const installedMergeTools = gitTools.filter(
    (t) => t.installed && t.supportsMerge,
  );
  ...
  const diffToolDisplay = (() => {
    ...
    const match = gitTools.find(
      (t) => t.id === currentDiffTool || t.id === token,
    );
    if (match) return { id: match.id, name: match.name };
  ...
  const mergeToolDisplay = (() => {
    ...
    const match = gitTools.find(
      (t) => t.id === currentMergeTool || t.id === token,
    );
    if (match) return { id: match.id, name: match.name };
  ...
  async function saveCustomEditor() {
    try {
      ...
      onToast(
        value ? `Editor set to "${value}"` : "Editor config cleared",
        "success",
      );
    } catch (err) {
      onToast(String(err), "error");
      ...
      {toolsLoading ? (
        ...
        <div className="divide-y divide-(--sg-border)">
          ...
          <div className="px-5 py-4">
            <div className="flex items-start justify-between">
              <div className="flex gap-2.5">
                ...
                <div>
                  <p className="sg-heading text-xs font-semibold text-(--sg-text)">
                    Author Identity
                  </p>
                  <p className="text-[11px] text-(--sg-text-faint)">
                    {currentGitName || "(not set)"} ·{" "}
                    {currentGitEmail || "(not set)"}
                  </p>
                </div>
              </div>
              <button
            ...
            {editingAuthor && (
              <div className="mt-3 space-y-2 border-t border-(--sg-border) pt-3">
                ...
                {githubAuth?.authenticated && (
                  <div className="flex flex-wrap gap-2">
                    {githubEmailsLoading ? (
                      <span className="text-[11px] text-(--sg-text-faint)">
                        Loading GitHub emails...
                      </span>
                    ) : (
          ...
          <SettingsToolRow
            ...
            presets={installedEditors.map((e) => ({
              id: e.id,
              name: e.name,
              active: !!currentEditor && matchesEditor(e, currentEditor),
            }))}
            onSelectPreset={(id) => {
          ...
          <SettingsToolRow
            ...
            presets={installedDiffTools.map((t) => ({
              id: t.id,
              name: t.name,
              active: currentDiffTool === t.id,
            }))}
            onSelectPreset={(id) => {
            ...
            onTest={async () =>
              api.testDiffTool(await resolveDiffToolTestCommand())
            }
            onToast={onToast}
          />
          ...
          <SettingsToolRow
            ...
            presets={installedMergeTools.map((t) => ({
              id: t.id,
              name: t.name,
              active: currentMergeTool === t.id,
            }))}
            onSelectPreset={(id) => {
            ...
            onTest={async () =>
              api.testMergeTool(await resolveMergeToolTestCommand())
            }
            onToast={onToast}
          />
        </div>
