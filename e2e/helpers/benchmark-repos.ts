...
export function createHeroMediaRepo() {
  const repoPath = createTestRepo('axiom', {
    files: {
      ...
      'src/ui/components/Button.tsx': 'export function Button({ label }: { label: string }) { return <button>{label}</button>; }\n',
  ...
  writeRepoFile(repoPath, 'src/auth/jwt.ts',
    'export function authenticate(token: string) {\n' +
    '  if (!token) throw new Error("Missing token");\n' +
    '  return { valid: true, sub: "user:1" };\n' +
    '}\n');
  commitAll(repoPath, 'feat(auth): add JWT validation with error handling', 2);
  writeRepoFile(repoPath, 'src/auth/refresh.ts', 'export function refreshToken(token: string) { return token; }\n');
  commitAll(repoPath, 'feat(auth): add refresh token support', 3);
  writeRepoFile(repoPath, 'tests/auth.test.ts', 'describe("auth", () => { it("validates JWT", () => {}); });\n');
  ...
  writeRepoFile(repoPath, 'src/ui/dashboard.tsx',
  ...
  writeRepoFile(repoPath, 'src/ui/components/MetricsPanel.tsx', 'export function MetricsPanel() { return <div className="metrics" />; }\n');
  commitAll(repoPath, 'feat(ui): add MetricsPanel component', 6);
  writeRepoFile(repoPath, 'src/ui/theme.ts', 'export const theme = { primary: "#1a8a5c", surface: "#fff" };\n');
  commitAll(repoPath, 'feat(ui): add design token configuration', 7);
  writeRepoFile(repoPath, 'tests/dashboard.test.tsx', 'describe("Dashboard", () => { it("renders", () => {}); });\n');
  ...
  writeRepoFile(repoPath, 'tests/auth.test.ts',
    'describe("auth", () => {\n' +
    '  it("validates JWT", () => {});\n' +
  ...
  checkout(repoPath, 'main');
  mergeNoFastForward(repoPath, 'hotfix/token-expiry', 'Merge hotfix/token-expiry: expired JWT patch', 22);

  // ── release/1.0 tag ─────────────────────────────────────────────────────────
  checkout(repoPath, 'release/1.0', true);
  writeRepoFile(repoPath, 'src/index.ts', 'export { createApp } from "./app";\nexport const VERSION = "1.0.0";\n');
  ...
  writeRepoFile(repoPath, 'src/notifications/index.ts', 'export function notify(msg: string) { console.log(msg); }\n');
  commitAll(repoPath, 'feat: add push notification service', 14);
  writeRepoFile(repoPath, 'src/notifications/preferences.ts', 'export const defaultPrefs = { email: true, push: true };\n');
  commitAll(repoPath, 'feat: add notification preference model', 15);
  writeRepoFile(repoPath, 'tests/notifications.test.ts', 'describe("notifications", () => { it("sends", () => {}); });\n');
  ...
  writeRepoFile(repoPath, 'vite.config.ts', 'import { defineConfig } from "vite";\nexport default defineConfig({});\n');
  ...
  writeRepoFile(repoPath, 'docs/CHANGELOG.md', '# Changelog\n\n## v1.1.0\n- Push notifications\n- Dependency upgrades\n');
  commitAll(repoPath, 'docs: update CHANGELOG for v1.1.0', 35);
  writeRepoFile(repoPath, 'src/config.ts', 'export const config = { env: "production", debug: false };\n');
  ...
  writeRepoFile(repoPath, 'src/api/v2/client.ts', 'export const API_V2 = "https://api.axiom.dev/v2";\n');
  commitAll(repoPath, 'feat(api): scaffold v2 client module', 11);
  writeRepoFile(repoPath, 'src/api/v2/schema.ts', 'export type ApiResponse<T> = { data: T; meta: object };\n');
  commitAll(repoPath, 'feat(api): add v2 response schema types', 12);
  writeRepoFile(repoPath, 'src/api/v2/batch.ts', 'export function batch(requests: unknown[]) { return requests; }\n');
  commitAll(repoPath, 'feat(api): add request batching support', 13);
  checkout(repoPath, 'main');

  checkout(repoPath, 'feature/dark-mode', true);
  writeRepoFile(repoPath, 'src/ui/theme.ts',
  ...
  writeRepoFile(repoPath, 'src/ui/useTheme.ts', 'export function useTheme() {\n  return window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";\n}\n');
  commitAll(repoPath, 'feat: add useTheme hook for automatic dark mode', 26);
  writeRepoFile(repoPath, 'tests/theme.test.ts', 'describe("theme", () => { it("detects dark mode", () => {}); });\n');
  commitAll(repoPath, 'test: add dark mode detection unit tests', 27);
  checkout(repoPath, 'main');

  checkout(repoPath, 'feat/settings', true);
  writeRepoFile(repoPath, 'src/ui/settings/index.tsx', 'export function SettingsPage() { return <div>Settings</div>; }\n');
  commitAll(repoPath, 'feat: add user settings page skeleton', 28);
  writeRepoFile(repoPath, 'src/ui/settings/storage.ts', 'export const saveSettings = (s: object) => localStorage.setItem("settings", JSON.stringify(s));\n');
  commitAll(repoPath, 'feat: persist settings to localStorage', 29);
  writeRepoFile(repoPath, 'tests/settings.test.ts', 'describe("settings", () => { it("persists", () => {}); });\n');
  commitAll(repoPath, 'test: add settings integration tests', 30);
  checkout(repoPath, 'main');

  checkout(repoPath, 'docs/api-reference', true);
  writeRepoFile(repoPath, 'docs/api.md',
  ...
  writeRepoFile(repoPath, 'docs/examples/quickstart.ts', 'import { createApp } from "axiom";\nconst app = createApp();\napp.listen(3000);\n');
